...
from commons.decoraters import redirect_to_home_if_authenticated
...
def user_account_activation(request, username, activation_key):
    if UserAccountActivationKey.objects.filter(user__username=username, key=activation_key).exists():
        ...
        return redirect('user_login')
    messages.error(request, "Invalid Activation Key, or the Link has Expired!")
    return redirect('user_login')
...
class UserProfileUpdateView(CreateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            # act_message = error_data.get('resume', [0])[0]
            # messages.error(request, error_data.get("resume"))
